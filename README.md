...
**A study in disappearance.** An avant-garde fashion-house landing page where a
black-and-white film of a walking figure lives in the page's negative space —
...
![Vite 7](https://img.shields.io/badge/vite-7-000000?logo=vite)
![Dependencies](https://img.shields.io/badge/runtime%20deps-react%20only-000000)

<img src="docs/media/01-hero.jpg" alt="OMBRA hero — the wordmark OMBRA over the display heading The Shape Remains, in bone white on black" width="100%">

</div>

---

Maison OMBRA is a **fictional house** — a concept piece built to see how far a
scroll-driven web page can be pushed with no animation library, no UI kit and no
runtime dependency other than React itself. Everything below is ~1,880 lines of
TypeScript, a 220-line stylesheet, and a single `requestAnimationFrame` loop.

## The signature

<table>
<tr>
<td width="50%"><img src="docs/media/02-apparition.jpg" alt="The apparition trail: the walking figure smeared into several fading ghosts behind the heading Silhouette"></td>
...
<tr>
<td><b>Apparition trail.</b> Scroll velocity smears the figure into fading ghosts of itself. Stop, and they dissolve in under a second.</td>
<td><b>Editorial grid.</b> A 12-column asymmetric layout with per-item parallax, staggered baselines and a slow couture zoom on hover.</td>
</tr>
<tr>
...
**pre-decoded JPEG sequence** into a `<canvas>` — perfectly smooth and exactly
scroll-locked.
...
→ ~57px/frame) visibly stutters. Frames average **6.7 KB** and are decoded on
demand — the browser keeps a bounded LRU decode cache, so density costs
bandwidth, not RAM.
...
lines up with frame *N* exactly. Raise or lower `FPS` in
`scripts/build-assets.sh` to trade smoothness against repo weight.
...
- **Smooth drift** (`?motion=drift`). The `<video>` plays natively and scroll
  ...
  build-time lookup.
- **Light seep.** A quarter-resolution, heavily blurred, `screen`-blended copy
...
src/
  ...
  index.css                design tokens, hover language, breakpoints, reduced motion
...
Deploying somewhere else? `npm run build` and serve `dist/` as static files —
or set `BASE_PATH` to whatever prefix you need.
...
[MIT](LICENSE) for the code. The media in `public/` is not original code — read
the note at the bottom of the licence before reusing it.
